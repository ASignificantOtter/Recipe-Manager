...
function toWeightInGrams(ingredient: IngredientInput): number | null {
  ...
  const hasCanonicalQuantity = !!canonicalQuantity && canonicalQuantity > 0;
  if (hasCanonicalQuantity && canonicalUnit === "g") return canonicalQuantity;

  if (hasCanonicalQuantity && canonicalUnit === "ml") {
